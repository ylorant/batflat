...
class Site extends SiteModule
{
    ...
    public function init()
    {
        ...
        $this->tpl->set('searchBox', $this->insertSearchBox());
    }
    ...
    public function getSearch($phrase, $index = 1)
    {
        ...
        if (strlen($phrase) < $phraseMinLength) {
            ...
            $pages->execute([$this->currentLanguage(), '%' . $phrase . '%', '%' . $phrase . '%']);
            ...
            $blog->execute([$this->currentLanguage(), 2, '%' . $phrase . '%', '%' . $phrase . '%']);
            ...
            if (!empty($rows) && (count($rows) >= $index)) {
                ...
                $page['content'] = $this->insertResults($rows[$pagination->offset()]) . $pagination->nav();
            } else {
    ...
    private function insertSearchBox(): string
    {
        return $this->draw('input.html');
    }

    private function insertResults(array $results): string
    {
